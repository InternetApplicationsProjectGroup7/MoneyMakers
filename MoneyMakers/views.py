...
from .forms import UserLoginForm, AccountRegistrationForm, UserProfileForm, AssetSellForm, PasswordResetForm, \
    AssetExchangeForm, FundsAdditionForm, UpdatePasswordForm
...
def handle_user_profile(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'FrontEnd/profile.html',
                      {'user': current_user, 'wish_list': user_wishlist, 'id': "profile-details"})
    ...
    if request.method == 'POST':
        ...
        if purchase_form.is_valid():
            ...
            if user_wallet.account_balance >= total_cost:
                ...
                user_crypto_holdings[chosen_crypto.name] = user_crypto_holdings.get(chosen_crypto.name, 0) + int(
                    purchase_quantity)
